...
module Cabal2Nix.Generate where
...
test :: IO PackageDescription
test = do
  cabal <- readPackageDescription silent "/home/simons/src/cabal2nix/cabal2nix.cabal"
  let Right (descr, _) = finalizePackageDescription
                           []
                           (const True)
                           (Platform X86_64 Linux)                 -- shouldn't be hardcoded
...
yo = do
  cabal <- test
  putStrLn $ render $ pPrint $ cabal2nix' cabal
